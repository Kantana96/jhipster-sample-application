...
import com.mycompany.simple_project.config.EmbeddedKafka;
import com.mycompany.simple_project.config.KafkaSseConsumer;
import com.mycompany.simple_project.config.KafkaSseProducer;
...
class JhipsterSampleApplicationKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/jhipster-sample-application-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/jhipster-sample-application-kafka/consume")
